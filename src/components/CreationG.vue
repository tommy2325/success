<template>
    <div class="popup-overlay">
      <div class="popup-content">
        ...
        <form @submit.prevent="createGroup">
          ...
          <label>Utilisateurs sans groupe:
            <select v-model="selectedUsers" multiple>
              <option v-for="user in usersWithoutGroup" :key="user.id_utilisateur" :value="user.id_utilisateur">
                {{ user.pseudo }}
              </option>
            </select>
          </label>
  ...
  const fetchUsersWithoutGroup = async () => {
    ...
    if (error) {
      console.error('Erreur lors de la récupération des utilisateurs sans groupe:', error);
    } else {
